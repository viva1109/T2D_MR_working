...
write.csv(data_ready3,"~/data_ready_SNP_MB.csv")
...
data_cbd_MBMGPH_bf<-cbind(data_ready3_2,data_ready3[match(RIDT_left,RIDT_right),],P=data_ready3_PH[match(RIDT_left,RIDT_right2),])

data_cbd_MBMGPH_bf$P.INS60_log<-log(data_cbd_MBMGPH_bf$P.INS60)
data_cbd_MBMGPH_bf$P.HBA1C_log<-log(data_cbd_MBMGPH_bf$P.HBA1C)
corM<-cor(data_cbd_MBMGPH_bf[,c("P.INS60_log","P.HBA1C_log","C34","R.X3.Hydroxybutyrate","R.Acetoacetate")],use="pairwise.complete.obs")
MM<-as.matrix(data_cbd_MBMGPH_bf[,c("P.INS60_log","P.HBA1C_log","C34","R.X3.Hydroxybutyrate","R.Acetoacetate")])

png("~/corplot2.png")
plot(data_cbd_MBMGPH_bf[,c("P.INS60_log","P.HBA1C_log","C34","R.X3.Hydroxybutyrate","R.Acetoacetate")])
dev.off()
...
library(stargazer)
ind_removeLOG<-which(str_detect(names(data_cbd_MBMGPH_bf),"ate_log$"))
data_cbd_MBMGPH<-data_cbd_MBMGPH_bf[,-ind_removeLOG]
SNP_ind<-which(str_detect(names(data_cbd_MBMGPH),"^rs"))

MG_RS_label<-names(data_cbd_MBMGPH)[SNP_ind[1:6]]
MB_RS_label<-setdiff(names(data_cbd_MBMGPH)[SNP_ind[7:length(SNP_ind)]],"rs2259835")
# i<-2

label_Y<-"R.X3.Hydroxybutyrate"
label_X<-"C34"
targetLabel<-MG_RS_label
library(mediation)

res_cb<-data.frame(matrix(nrow=length(targetLabel),ncol=12))

res_cb$Y<-label_Y
res_cb$X<-label_X
res_cb$SNP<-targetLabel
library(nlme)
library(ivreg)
for ( i in 1:length(targetLabel)){
  tmpX <- data_cbd_MBMGPH[,targetLabel[i]]
  M <- data_cbd_MBMGPH[,label_X]
  Y <- data_cbd_MBMGPH[,label_Y]
  
  DATA_lme<-data.frame(X=tmpX, M, Y,AGE=data_cbd_MBMGPH$R.AGE,SEX=data_cbd_MBMGPH$SEX,IID=data_cbd_MBMGPH$IID)
  rm(list=c("X","M","Y"))
  
  m1.nlme_M = lme(M ~ factor(SEX) + AGE,random = ~ 1|IID,data = DATA_lme[complete.cases(DATA_lme),])                    
  res_M<-residuals(m1.nlme_M)
  
  #M <- combined5$M_Acetoacetate
  
  m1.nlme_Y = lme(Y ~ factor(SEX) + AGE,random = ~ 1|IID,data = DATA_lme[complete.cases(DATA_lme),])                    
  res_Y<-residuals(m1.nlme_Y)
  Meddata <- data.frame(X=DATA_lme$X[complete.cases(DATA_lme)], res_M, res_Y)
  Meddata2<-Meddata[complete.cases(Meddata),]
  

  
  
  m_iv <- ivreg(res_Y ~ res_M | X,  data = Meddata2)
  summary(m_iv)
  ...
  library(mediation)
  fitM <- lm(res_M ~ X,     data=Meddata2) #IV on M; Hours since dawn predicting coffee consumption
  fitY <- lm(res_Y ~ X + res_M, data=Meddata2) #IV and M on DV; Hours since dawn and coffee predicting wakefulness
  # gvlma(fitM) #data is positively skewed; could log transform (see Chap. 10 on assumptions)
  fitMed <- mediate(fitM, fitY, treat="X", mediator="res_M")
  # result_out<-summary(fitMed)
  makeCIexp<-function(CItmp){
    paste0("(",paste(round(CItmp*10000)/10000,collapse=", "),")")
  }
  roundid<-function(value,power=4){
    round(value*10^power)/10^power
  }
  val_res<-c(c(roundid(fitMed$tau.coef),makeCIexp(fitMed$tau.ci),roundid(fitMed$tau.p))
  ,c(roundid(fitMed$d0),makeCIexp(fitMed$d0.ci),roundid(fitMed$d0.p))
  ,c(roundid(fitMed$z0),makeCIexp(fitMed$z0.ci),roundid(fitMed$z0.p))
  ,c(roundid(fitMed$n0),makeCIexp(fitMed$n0.ci),roundid(fitMed$n0.p)))
  
  
  
  res_cb[i,1:12]<-val_res
}


names(res_cb)[1:12]<-c(paste0("Total_",c("Estimate","CI","P.value")),paste0("M_",c("Estimate","CI","P.value")),paste0("D_",c("Estimate","CI","P.value")),paste0("Prop_",c("Estimate","CI","P.value")))

write.csv(res_cb,paste0("~/",label_X,"_",label_Y,"_MED0915.csv"))
system(paste0("scp ",paste0("~/",label_X,"_",label_Y,"_MED0915.csv ng:~")))
